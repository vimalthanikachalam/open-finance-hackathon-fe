...
router.get("/account-access-consents", requireAccessToken, async (req, res) => {
  const interactionId = uuidv4();
  const authHeader = req.headers["authorization"];

  const url = `${config.RESOURCE_SERVER}/open-finance/account-information/v1.2/account-access-consents`;
